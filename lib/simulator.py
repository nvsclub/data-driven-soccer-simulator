...
def load_models():
    ...
    #dribble = pickle.load(open('matrix/D.sav', 'rb'))
    #dribble = None
    ...
    shots = pickle.load(open('matrix/shot_gradient.sav', 'rb'))
    ...
    return passes, shots#, dribble, rebound_data, rebound

passes, shots = load_models()
...
class Dribble:
    ...
    def simulate(self, verbose):
        prob = dribble.predict_proba([[self.x, self.y, self.xt, self.yt]])[0,0]
        if random() < prob and (abs(self.x - self.xt) + abs(self.y - self.yt)) <= 25:
            self.success = True
            if verbose: print('Successful Dribble', self.success, prob, self.x, self.y, self.xt, self.xt)
            return self.xt, self.yt
        else:
            if verbose: print('Unsuccessful Dribble', self.success, prob, self.x, self.y, self.xt, self.xt)
            if ENABLE_REBOUND:
                self.rebound = Rebound(self.x, self.y)
                return self.rebound.simulate()
            else:
                return self.x, self.y
'''
...
class Pass:
    def __init__(self, x, y, r, a, action_reward, backwards_reward, out_of_bounds_reward):
    ...
    def simulate(self, verbose):
        self.reward = shots.predict_proba([[self.xt, self.yt]])[0,1] / 10 + self.action_reward
        ...
        if random() < self.prob:
            self.success = True
            if verbose:
                print('Successful Pass', self.success, self.prob, self.x, self.y, self.r, self.a)
            if self.x > self.xt:
            ...
            if verbose:
                print('Unuccessful Pass', self.success, self.prob, self.x, self.y, self.r, self.a)
            if self.x > self.xt:
                return self.xt, self.yt, self.backwards_reward
            else:
                return self.xt, self.yt, self.reward
...
class Shot:
    ...
    def simulate(self, verbose):
        if self.x < 0.5:
            return self.x, self.y
        self.xg = shots.predict_proba([[self.x, self.y]])[0,1]
...
class Agent:
    ...
    def do_shot(self, r=0, a=0, verbose=False):
        self.actions.append(Shot(self.x, self.y))
        self.x, self.y = self.actions[-1].simulate(verbose)
        self.xg = self.actions[-1].xg
        self.goal, self.end = evaluate_goal(self.actions[-1])

        if self.end_on_xg:
            return [self.x, self.y], self.actions[-1].xg, self.end, [1, 0, r, a]

        return [self.x, self.y], self.xg, self.end, [1, 0, r, a]
    
    def do_pass(self, r, a, verbose=False):
        self.actions.append(Pass(self.x, self.y, r, a, action_reward = self.action_reward, backwards_reward = self.backwards_reward, out_of_bounds_reward = self.out_of_bounds_reward))
        self.x, self.y, reward = self.actions[-1].simulate(verbose)
        self.rewards += reward
        self.end = evaluate_rebound_end(self.actions[-1])

        return [self.x, self.y], reward, self.end, [0, 1, r, a]

    '''def do_dribble(self, xt, yt, verbose=False):
        self.actions.append(Dribble(self.x, self.y, xt, yt))
        self.x, self.y = self.actions[-1].simulate(verbose)
        self.end = evaluate_rebound_end(self.actions[-1])

        return [self.x, self.y], self.goal, self.end, [0, 0, 1, xt, yt]'''
    ...
    def do_action(self, action, r=None, a=None, xt=None, yt=None, verbose=False):
        if not self.end:
            ...
            if action == 0:
                ...
                return next_obs, reward, is_done, action_used
            '''elif action == 2:
                next_obs, reward, is_done, action_used = self.do_dribble(xt, yt, verbose)
                return next_obs, reward, is_done, action_used'''
        else:
...
def draw_actions(agent, action_no):
    ...
    all_patches = []
    all_patches.append(Wedge((x,y), distance, angle-0.5, angle+0.5, fc='white', zorder = 11 + 4 * action_no))
    all_patches.append(Arc((x,y), 3, 3*horizontal_scalling, 0, angle-45, angle+45, lw = 7 * action_sample.success, ec='#611a6a', zorder = 10 + 4 * action_no))
    all_patches.append(Arc((x,y), 3, 3*horizontal_scalling, 0, angle-45, angle+45, lw = 7, ec='white', zorder = 9 + 4 * action_no))
...
def draw_actions_for_set(agent_set, action_no):
    ...
    all_patches = []
    all_patches.append(Wedge((x,y), distance, angle-0.5, angle+0.5, fc='white', zorder = 11 + 4 * action_no))
    all_patches.append(Arc((x,y), 3, 3*horizontal_scalling, 0, angle-45, angle+45, lw = 7 * success_count/len(action_set), ec='#611a6a', zorder = 10 + 4 * action_no))
    all_patches.append(Arc((x,y), 3, 3*horizontal_scalling, 0, angle-45, angle+45, lw = 7, ec='white', zorder = 9 + 4 * action_no))
